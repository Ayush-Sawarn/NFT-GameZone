...
const PlayerInfo = ({ player, playerIcon, mt }) => {
  const { contract, gameData, showAlert, setshowAlert, battleGround, setBattleGround, walletAddress, setErrorMessage, player1Ref, player2Ref } = useGlobalContext();

  return (
    <div className={`${styles.flexCenter} ${mt ? 'mt-4' : 'mb-4'}`}>
      <img data-for={`Player-${mt ? '1' : '2'}`} data-tip="" src={playerIcon} alt="player" className="w-14 h-14 object-contain rounded-full" />
